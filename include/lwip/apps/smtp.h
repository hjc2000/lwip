...
#define SMTP_BODYDH_BUFFER_SIZE 256
    ...
    err_t smtp_send_mail_bodycback(char const *from, char const *to, char const *subject,
                                   smtp_bodycback_fn bodycback_fn, smtp_result_fn callback_fn, void *callback_arg);

#endif /* SMTP_BODYDH */

    err_t smtp_set_server_addr(char const *server);
    void smtp_set_server_port(u16_t port);
#if LWIP_ALTCP && LWIP_ALTCP_TLS
    struct altcp_tls_config;
    void smtp_set_tls_config(struct altcp_tls_config *tls_config);
#endif
    err_t smtp_set_auth(char const *username, char const *pass);
    err_t smtp_send_mail(char const *from, char const *to, char const *subject, char const *body,
                         smtp_result_fn callback_fn, void *callback_arg);
    err_t smtp_send_mail_static(char const *from, char const *to, char const *subject, char const *body,
                                smtp_result_fn callback_fn, void *callback_arg);
    void smtp_send_mail_int(void *arg);
#ifdef LWIP_DEBUG
    char const *smtp_result_str(u8_t smtp_result);
#endif
